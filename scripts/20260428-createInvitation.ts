...
// 例) DRY_RUN=false npx tsx scripts/20260428-createInvitation.ts
...
(async () => {
  ...
  const query = db.collection('circles').where("eventId", "==", "gishohaku13")
  ...
  snapshot.docs.forEach(async doc => {
    ...
    console.log([
      ...
      name,
      token,
      `https://gishohaku.dev/gishohaku13/mypage/join?circleId=${doc.id}&token=${token}`
    ].join(", "))
  })
})()